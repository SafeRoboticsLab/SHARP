...
function [h_, hDraw_] = plot_vehicle(x, varargin)
    ...
    if isempty(opt.handle)
        ...
        % create the robot
        [h.vehicle, hDraw] = draw_robot(d, opt, args);
...
function [h, hDraw] = draw_robot(d, opt, args)
    ...
    if ~isempty(opt.model)
        ...
        hDraw = image(img, 'AlphaData', alpha, 'Parent', h2);
        %axis equal
    else
